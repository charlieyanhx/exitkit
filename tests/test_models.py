...
def test_model_refuses_empty_market_data(family, cls, window, position):
    ...
    model = cls()
    try:
        signals = model.generate_exit_signals(window, [position()], {})
...
def test_model_runs_on_complete_market_data(family, cls, window, position, market):
    model = cls()
    signals = model.generate_exit_signals(window, [position()], market())
...
def test_no_positions_means_no_signals(family, cls, window, market):
    assert cls().generate_exit_signals(window, [], market()) == []
...
def test_stop_loss_fires_on_a_loss(window, position, market):
    ...
    signals = m.generate_exit_signals(window, [p], down)
...
def test_stop_loss_holds_on_a_small_move(window, position, market):
    ...
    signals = m.generate_exit_signals(window, [position()], market(spot_price=399.0))
    assert signals == []
...
def test_take_profit_fires_on_a_gain(window, position, market):
    ...
    signals = m.generate_exit_signals(window, [position()], market(spot_price=420.0))
...
def test_fixed_time_exit_respects_its_horizon(window, position, market):
    ...
    assert m.generate_exit_signals(window, [position(held_hours=0.5)], market()) == []
    late = m.generate_exit_signals(window, [position(held_hours=9.0)], market())
    assert len(late) == 1
...
def test_a_short_position_loses_when_price_rises(window, position, market):
    ...
    signals = m.generate_exit_signals(window, [short], market(spot_price=420.0))
...
def test_signals_carry_the_position_they_close(window, position, market):
    ...
    signals = m.generate_exit_signals(window, [position()], market(spot_price=350.0))
